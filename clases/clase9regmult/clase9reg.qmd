...
subtitle: "Sesión 9: Regresión Múltiple"
format:
  revealjs:
    theme: default
...
scatterm + geom_point(size = 4) + labs(x = "Logro", y = "Estatus") + scale_y_continuous(limits=c(2, 6), breaks=c(2:6)) + scale_x_continuous(limits=c(2, 7), breaks=c(2:7))
```
...
```
| Estatus(X)     | Logro(Y)       | $(X-\bar{X})$ | $(Y-\bar{Y})$ | $(X-\bar{X})^2$ | $(Y-\bar{Y}^2$  | $(X-\bar{X})*(Y-\bar{Y})$ |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
